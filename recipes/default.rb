...
if node['kyotocabinet']['user'] != 'root'
  directory node['kyotocabinet']['shell_d'] do
    owner node['kyotocabinet']['user']
  ...
  bash "add_include_directive" do
    ...
    EOH
    not_if "grep 'Generated by Chef add_include_directive' /home/#{node['kyotocabinet']['user']}/.bashrc"
  end
end
...
bash "kyotocabinet_set_variables" do
  ...
  export PATH=#{node['kyotocabinet']['src']['prefix']}/bin:$PATH
  export LD_LIBRARY_PATH="#{node['kyotocabinet']['src']['prefix']}/lib:$LD_LIBRARY_PATH"
  export PKG_CONFIG_PATH="#{node['kyotocabinet']['src']['prefix']}/lib/pkgconfig:PKG_CONFIG_PATH"
  EOF
